...
The unit is one resample, returning one row. The same property as the previous page: nothing in it knows where it will run.
...
## Jittering coordinates, not geometry
...
If the local run finishes, stop reading here. What follows describes what it took to run this at a size where local stopped being enough, and it is not a recommendation to skip ahead.
...
Three things about the worker decide whether this runs at all.

`sf` has to load there, and it is not on a stock cluster image. Where it is available, an administrator installed it on every node, and an ad-hoc install in a notebook does not achieve that because it touches one machine. [Packages](../ref/packages.qmd) and [Setting up a cluster for geospatial R](../admin/geospatial-setup.qmd) are the two ends of that conversation.

The backend inside the worker is `parallel`, not `furrr`, which stage four settled and this is where it matters.
...
This is the check the whole page turns on, and the one most examples omit.
