...
#| label: mask
#| include: false
# This site is published, so a rendered page must not
# carry the catalog this example runs against. Queries
# below all use the real `catalog`; shown() is applied
# only to text on its way to being printed.
shown <- function(x) {
  gsub(catalog, "your_catalog", x, fixed = TRUE)
}
```
...
longest_five <- tbl(con, I(tbl_name("wfd_catchments"))) |>
