...
The two runs are not the same size, so they are not comparable as timings. Both are single runs in a session with `r n_cores` core`r if (n_cores != 1) "s"` available, quoted so the numbers mean something rather than as a benchmark.
