...
- **"Her data is in Databricks" does not tell you whether it is a table or a file.** Both are ordinary, and the difference decides her first line of code. A governed table she reaches with `tbl()`; a CSV, workbook or shapefile sitting in a volume somebody else populated she has to fetch before R can open it, because `/Volumes/...` is not a path in her session. Observed teams sit at both ends, and one uses Databricks almost entirely as a file store. So never assume a table: say which of the two a page is about, and never let a page's first step assume the shape she does not have.
